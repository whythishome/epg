...
module.exports = {
  site: 'repo-server.site',
  days: 2,
  url({ channel }) {
    const [tag] = channel.site_id.split('#')

    return `https://github.com/ferteque/Curated-M3U-Repository/raw/refs/heads/main/epg${tag}.xml.gz`
  },
  request: {
    cache: {
  ...
  async channels({ tag }) {
    const buffer = await axios
      .get(`https://github.com/ferteque/Curated-M3U-Repository/raw/refs/heads/main/epg${tag}.xml.gz`, {
